...
@Operation(
    ...
    public ResponseEntity<ResponseDto> createUser(@RequestBody UserDto userModule) {
        try {
            ...
            var createdUser = userModuleService.createUser(userModule);
            if (createdUser != null) {
            ...
            log.error("UserController:createUser - Error while creating user account for email: {}", userModule.getEmail(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
@Operation(
